...
describe("LocationUtil (Haversine Distance)", () => {
  ...
  it("should return null if any coordinate is null or undefined", () => {
    expect(
      LocationUtil.calculateDistanceKm(null, 77.209, 28.6139, 77.209),
    ).toBeNull();
    expect(
      LocationUtil.calculateDistanceKm(28.6139, undefined, 28.6139, 77.209),
    ).toBeNull();
    expect(
      LocationUtil.calculateDistanceKm(28.6139, 77.209, null, null),
    ).toBeNull();
  });

  it("should return null for out-of-range coordinates", () => {
    expect(
      LocationUtil.calculateDistanceKm(95, 77.209, 28.6139, 77.209),
    ).toBeNull();
    expect(
      LocationUtil.calculateDistanceKm(28.6139, 190, 28.6139, 77.209),
    ).toBeNull();
    expect(
      LocationUtil.calculateDistanceKm(28.6139, 77.209, -91, 0),
    ).toBeNull();
  });

  it("should return null for non-numeric coordinates", () => {
    expect(
      LocationUtil.calculateDistanceKm(NaN, 77.209, 28.6139, 77.209),
    ).toBeNull();
  });
});
